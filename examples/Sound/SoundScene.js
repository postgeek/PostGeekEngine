...
export default class SoundScene extends Scene {
  ...
  create() {
    ...
    this.currentAudioTimeText = new TextGraphic(new Vec2D(20, 20), '');
    this.currentAudioTimeText.textStyle = new TextStyle({
      strokeStyle: Color.WHITE,
    });
    this.durationTimeText = new TextGraphic(new Vec2D(20, 30), '');
    this.durationTimeText.textStyle = new TextStyle({
      strokeStyle: Color.WHITE,
    });
    this.currentStateText = new TextGraphic(new Vec2D(20, 40), '');
    this.currentStateText.textStyle = new TextStyle({
      strokeStyle: Color.WHITE,
    });

    this.soundBoardText = new TextGraphic(new Vec2D(20, 160), 'Sound Board');
    ...
    this.playPauseButton = new Button(new Vec2D(20, 60), 'Play', (event) => this.playOrResumeSound(event));
    this.stopButton = new Button(new Vec2D(80, 60), 'Stop', (event) => this.stopSound(event));

    this.playPauseButton.disabled = true;
    this.stopButton.disabled = true;

    this.soundBoardChipPowerButton = new Button(new Vec2D(20, 180), 'CP', () => this.playChipPower());
    this.soundBoardSonarButton = new Button(new Vec2D(60, 180), 'Sonar', () => this.PlaySonar());
    this.soundBoardCrystalButton = new Button(new Vec2D(120, 180), 'Crystal', () => this.PlayCrystal());

    this.nodeTypeText = new TextGraphic(new Vec2D(200, 30), 'Node');
    ...
    for (let i = 0; i < 2; i++) {
      this.nodeButtons.push(
        new Button(new Vec2D(280 + i * 25, 10), `${i + 1}`, () => {
          this.setNodeType(i);
          for (let i = 0; i < 2; i++) {
    ...
    this.volumeLeftText = new TextGraphic(new Vec2D(200, 70), 'Volume (L)');
    ...
    for (let i = 0; i < 10; i++) {
      this.volumeButtonsLeft.push(
        new Button(new Vec2D(310 + i * 25, 50), `${i + 1}`, () => {
    ...
    this.volumeRightText = new TextGraphic(new Vec2D(200, 110), 'Volume (R)');
    ...
    for (let i = 0; i < 10; i++) {
      this.volumeButtonsRight.push(
        new Button(new Vec2D(310 + i * 25, 90), `${i + 1}`, () => {
